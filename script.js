...
const startAudio = () => {
  ...
  player = new Tone.Player(
    "https://s3-us-west-2.amazonaws.com/s.cdpn.io/67732/open-e-down.m4a"
  )
    .connect(dist)
    .sync()
    .start(0);
...
function detectPoseInRealTime(video, net) {
  ...
  eltxt = document.createTextNode(
    "Stand in a spot where the camera can see your full torso as well as both arms. Your left arm controls the pitch of the notes - the further away it is from your body on the white line, the lower the notes. Move your right hand across the white line to strum the strings."
  );
  el.appendChild(eltxt);
  document.querySelector("#overlay div").appendChild(el);
...
function playGuitar(points) {
  ...
  if (
    points[indexes.fretWrist].score > 0.4 &&
    points[indexes.pickWrist].score > 0.4
  ) {
    ...
    pickRatio =
      Math.hypot(
        parts.pickForearm.x2 - parts.pickForearm.x1,
        parts.pickForearm.y2 - parts.pickForearm.y1
      ) * 0.005;
    pick = {
      x:
        parts.pickForearm.x2 +
        (parts.pickForearm.x2 - parts.pickForearm.x1) * pickRatio,
      y:
        parts.pickForearm.y2 +
        (parts.pickForearm.y2 - parts.pickForearm.y1) * pickRatio,
    };
    ...
    neckAngle = getRadians(
      hipCenter.x,
      hipCenter.y,
      parts.wrist.x,
      parts.wrist.y
    );
    neckStart = findNewPoint(
      hipCenter.x,
      hipCenter.y,
      neckAngle,
      torsoHeight * 0.3
    );
    neckEnd = findNewPoint(
      hipCenter.x,
      hipCenter.y,
      neckAngle,
      torsoHeight * 1.5
    ...
    if (lastPickPos) {
      ...
      if (strummed && pick.y < lastPickPos.y) {
        handPosition = Math.round(
          (1 -
            Math.sqrt(
              Math.pow(parts.wrist.x - neckStart.x, 2) +
                Math.pow(parts.wrist.x - neckStart.y, 2)
            ) /
              Math.sqrt(
                Math.pow(neckEnd.x - neckStart.x, 2) +
                  Math.pow(neckEnd.x - neckStart.y, 2)
              )) *
            22
        );
